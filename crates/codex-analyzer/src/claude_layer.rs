...
pub fn generate_metadata_header(version: &TreeVersion) -> String {
    format!(
        ...
        version.tree_version.generation, version.tree_version.delta_count, version.format_version,
    )
}
...
pub fn generate_l1(tree: &TreeStructure, modules: &[ModuleIndex], version: &TreeVersion) -> String {
    ...
    writeln!(
        ...
        project_name, version.tree_version.generation, version.tree_version.delta_count,
    )
    .unwrap();
    writeln!(buf).unwrap();
    ...
    writeln!(
        buf,
        "| Lines of code | {} |",
        version.stats.total_lines_of_code
    )
    .unwrap();
    writeln!(
        buf,
    ...
    match intent {
        Some(intent_data) if !intent_data.patterns.patterns.is_empty() => {
            writeln!(
                buf,
                "_Generated by {} on {}_",
                intent_data.patterns.generator_model, intent_data.patterns.generated_at,
            )
            .unwrap();
            writeln!(buf).unwrap();
    ...
    match intent {
        Some(intent_data) if !intent_data.decisions.decisions.is_empty() => {
            writeln!(
                buf,
                "_Generated by {} on {}_",
                intent_data.decisions.generator_model, intent_data.decisions.generated_at,
            )
            .unwrap();
            writeln!(buf).unwrap();
...
fn top_level_dirs(tree: &TreeStructure) -> Vec<&TreeEntry> {
    tree.entries
        .iter()
        .filter(|e| matches!(e, TreeEntry::Directory { path, .. } if !path.contains('/')))
        .collect()
}
...
mod tests {
    ...
    fn write_claude_layer_creates_files() {
        ...
        assert_eq!(
            fs::read_to_string(dir.path().join("claude/l1.md")).unwrap(),
            l1
        );
        assert_eq!(
            fs::read_to_string(dir.path().join("claude/l2.md")).unwrap(),
            l2
        );
        assert_eq!(
            fs::read_to_string(dir.path().join("claude/l3.md")).unwrap(),
            l3
        );
    }
